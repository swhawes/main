...
!!!!!!!!!!!!!!!!!!DF_LONG HAS NO YEAR_2 DATA INCLUDED- START HERE on 12/20
```{r install}
...
recall_anova <-lm(formula = cbcl_agg ~ vg_total, data = df_long)
random_intercepts <- apa_print(recall_anova, es = "pes")
random_intercepts_p <- apa_print(recall_anova, es = "pes", in_paren = TRUE)

random_intercepts
random_intercepts_p

## analysis when filtering by mature video users
#df_long_filtered<-df_long[!(df_long_filtered$mature_vg=="no"),]
#random_intercepts <- lmer(cu_traits ~ 1 + event + vg_total + sex + (1|ids), data #= df_long_filtered, REML=T)
...
model <- ' i =~ 1*cbcl_agg_T0 + 1*cbcl_agg_T1 + 1*cbcl_agg_T2
           s =~ 0*cbcl_agg_T0 + 1*cbcl_agg_T1 + 2*cbcl_agg_T2
           
# regressions
    i ~ mature_vg_T0*vg_total_T0 #sex + age + edupar + incpar
    s ~ mature_vg_T0*vg_total_T0 #sex + age + edupar + incpar
